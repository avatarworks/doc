...
SDK 完全跑在一个独立的线程上，从而使得 SDK 的内部操作，在一般情况下不影响主线程（或UI线程）的性能。但正如所有异步操作可能带来的同步问题一样，开发者在主线程更新SDK的时候，也不可避免的要注意线程同步问题。为了方便开发者使用，SDK 会将每一步更新操作，都会丢入一个 FIFO 队列中，以尽可能地方便开发者可以不需要在上一个更新操作的回调函数中去处理下一次更新操作。同时，SDK 还提供了解决队列拥堵的机制，即当前一个更新操作因为耗时而堵塞队列时，后面 **同类型** 的更新操作会自动合并成一个大的更新操作，从而在前一个操作结束以后，直接将队列后面遗留的操作直接同步到最终想要的状态。例如，
